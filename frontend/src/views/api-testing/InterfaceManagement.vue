      ...
      <div class="sidebar">
        ...
        <div class="collection-tree">
          <template v-if="isSearching">
            <StateLoading
              v-if="searchState === UI_PAGE_STATE.LOADING"
              compact
            />
            <StateForbidden
              v-else-if="searchState === UI_PAGE_STATE.FORBIDDEN"
              compact
              :description="searchErrorMessage || $t('common.uiState.forbidden.description')"
              :primary-action-text="$t('common.uiState.actions.goHome')"
              :secondary-action-text="$t('common.uiState.actions.resetSearch')"
              @primary-action="router.push('/home')"
              @secondary-action="clearSearch"
            />
            <StateError
              v-else-if="searchState === UI_PAGE_STATE.REQUEST_ERROR"
              compact
              :description="searchErrorMessage || $t('common.uiState.error.description')"
              :secondary-action-text="$t('common.uiState.actions.resetSearch')"
              @primary-action="retrySearch"
              @secondary-action="clearSearch"
            />
            <StateSearchEmpty
              v-else-if="searchState === UI_PAGE_STATE.SEARCH_EMPTY"
              compact
              :primary-action-text="$t('common.uiState.actions.resetSearch')"
          ...
          <template v-else>
            <StateLoading
              v-if="sidebarState === UI_PAGE_STATE.LOADING"
              compact
            />
            <StateForbidden
              v-else-if="sidebarState === UI_PAGE_STATE.FORBIDDEN"
              compact
              :description="sidebarErrorMessage || $t('common.uiState.forbidden.description')"
              :primary-action-text="$t('common.uiState.actions.goHome')"
            ...
            />
            <StateError
              v-else-if="sidebarState === UI_PAGE_STATE.REQUEST_ERROR"
              compact
              :description="sidebarErrorMessage || $t('common.uiState.error.description')"
              @primary-action="retrySidebarLoad"
            />
            <StateEmpty
              v-else-if="sidebarState === UI_PAGE_STATE.EMPTY"
              compact
              :primary-action-text="$t('apiTesting.interface.createCollection')"
              :secondary-action-text="$t('apiTesting.interface.addInterface')"
              @primary-action="openCreateCollectionDialog"
              @secondary-action="createEmptyRequest"
            />
            <el-tree
              v-else
              ref="treeRef"
...
const sidebarState = ref(UI_PAGE_STATE.LOADING)
const searchState = ref(UI_PAGE_STATE.READY)
...
const isSearching = computed(() => Boolean(searchKeyword.value && searchKeyword.value.trim()))
...
const retrySidebarLoad = async () => {
  if (selectedProject.value) {
    await onProjectChange(selectedProject.value)
    return
  }
...
const onSearch = async (value) => {
  ...
  if (!selectedProject.value) {
    ...
    searchState.value = UI_PAGE_STATE.SEARCH_EMPTY
    searchErrorMessage.value = ''
    return
  }

  searchState.value = UI_PAGE_STATE.LOADING
  searchErrorMessage.value = ''
  ...
  try {
    ...
    filteredCollections.value = []
    searchState.value = error.response?.status === 403 ? UI_PAGE_STATE.FORBIDDEN : UI_PAGE_STATE.REQUEST_ERROR
    searchErrorMessage.value = error.response?.data?.detail || error.message || ''
...
const onProjectChange = async (projectId) => {
  if (!projectId) {
    ...
    sidebarState.value = UI_PAGE_STATE.EMPTY
    clearSearch()
    return
  }
...
const loadProjects = async () => {
  sidebarState.value = UI_PAGE_STATE.LOADING
  ...
  try {
    ...
    flatCollections.value = []
    selectedRequest.value = null
    sidebarState.value = error.response?.status === 403 ? UI_PAGE_STATE.FORBIDDEN : UI_PAGE_STATE.REQUEST_ERROR
    sidebarErrorMessage.value = error.response?.data?.detail || error.message || ''
...
const loadCollections = async (projectId) => {
  ...
  try {
    ...
    // 加载请求
    await loadRequests()
    sidebarState.value = collections.value.length ? UI_PAGE_STATE.READY : UI_PAGE_STATE.EMPTY
  } catch (error) {
    collections.value = []
    flatCollections.value = []
    selectedRequest.value = null
