...
5. `.cursor/autonomy_rules.md`
6. `.cursor/loop_rules.md`
7. `.cursor/architecture.md`
8. `.cursor/storage_rules.md`
9. `.cursor/project_rules.md`
10. `docs/project-memory/current_phase.md`
11. 代码库现状与最小惊扰原则

补充说明：

- `workflow_rules.md` 只负责流程闸门、任务分级、验证口径与文档回写，不得用于突破 `architecture.md` 的架构红线
- `autonomy_rules.md` 只负责 AI 决策权限、自治等级和用户确认边界，不得用于突破 `architecture.md` 的架构红线
- `loop_rules.md` 只负责受控 Loop 的进入条件、失败处理、验证等级和停止条件，不得用于扩大任务范围或降低验证标准
...
- `.cursor/*.md`
- 根 `AGENTS.md`
- 只定义当前仓库长期有效的规则、自治边界、Loop 边界、架构边界、存储边界与项目实现细则
...
- 记录当前阶段事实、冻结方案、验收口径、错误事件、防复发规则、Loop 执行记录和下一步主线
...
- 开始实现前必须判断自治等级 L0-L4，并明确是否允许受控 Loop
- 面向用户的需求确认、`Spec/SDD`、`TDD`、`VDD` 和交付说明必须使用中文大白话、细颗粒度表达
