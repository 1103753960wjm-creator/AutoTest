# 全局开发规则与项目基线

本文件是 AI 助手的全局记忆，用于约束跨项目的默认开发行为，以及当前 `AutoTest / L-Tester` 仓库的规则优先级。

---

## 1. 全局提示规则

你是当前仓库的开发助手。默认目标不是写“理想模板代码”，而是在现有架构下交付可运行、可验证、可维护的改动。

### 规则优先级
发生冲突时，按以下顺序执行：
1. 用户当前回合的明确要求
2. 全局记忆文件 `GEMINI.md`
3. 项目内 `.antigravity/prompt.md`
4. 项目内 `.antigravity/workflow_rules.md`
5. 项目内 `.antigravity/architecture.md`
6. 项目内 `.antigravity/storage_rules.md`
7. 项目内 `.antigravity/project_rules.md`
8. 代码库现状与最小惊扰原则

补充说明：
- `workflow_rules.md` 优先级提高，表示当前项目默认按 TDD/验证优先的开发流程执行。
- `workflow_rules.md` 约束的是实现顺序、验证方式和交付方式，不得用于突破 `architecture.md` 中已经明确写死的架构红线。

### 默认行为
- 先读相关代码，再做判断。
- 优先复用现有目录、工具函数、返回结构、接口封装。
- 不为了“显得先进”替换当前技术路线。
- 不把一次需求扩展成全仓重构。

### 外部文档与 MCP
- 当用户明确要求 use context7，或任务依赖某个框架/库的官方文档实现细节时，优先使用 Context7 MCP，不凭记忆直接实现。
- 使用顺序固定为：先 Resolve Context7 Library ID，再 Query Documentation。
- 通过 Context7 获取资料时，优先参考官方文档、官方示例和官方推荐写法。
- 如果用户明确写出类似以下指令，视为必须走 Context7：
- Create a Next.js middleware that checks for a valid JWT in cookies and redirects unauthenticated users to /login. use context7
- Configure a Cloudflare Worker script to cache JSON API responses for five minutes. use context7
- 这类任务的交付默认要体现“依据官方文档实现”，不要只给经验写法。
- 如果用户没有显式写 use context7，但任务明显依赖官方最新文档、框架约定、SDK/云平台配置或版本化 API，允许 AI 自动判断并优先调用 Context7 MCP。
- 自动调用 MCP 的前提是：本地代码、项目规范和现有上下文不足以稳定确定正确实现；如果本地仓库已经有明确模式或任务主要是仓内逻辑改动，则先读本地代码，不默认调用 MCP。
- 不允许把“自动调用 MCP”扩展成“所有技术问题一律查文档”；只有当官方文档对正确实现有决定性影响时才触发。

### 沟通与文档要求
- 与用户沟通统一使用中文。
- 所有新增或修改的规范文件、设计文档、任务文档、交付说明统一使用中文。
- 所有新增或修改的代码注释统一使用中文；字段名、协议关键字、库名、框架名和第三方服务名可保留英文原文。
- 输出尽量直接，先说结果和风险，再补细节。
- 无法验证时必须明确说明原因。

---

## 2. 当前项目工程规则

### 项目结构
- `l-tester-master` 是 Python 后端，技术栈为 FastAPI + Tortoise ORM + MySQL。
- `l-vue-ui-master` 是前端管理台，技术栈为 Vue 3 + TypeScript + Vite + Pinia + Element Plus。
- 这是前后端分离测试平台，不是通用后台模板；任何改动都要考虑脚本执行、结果落库、报表、通知这些业务闭环。

### 后端边界
- 路由函数不得堆积复杂业务逻辑。
- `*_model.py` 只做数据定义，不写业务流程。
- 新接口优先复用统一返回结构：`{"code": ..., "message": ..., "data": ...}`。
- 执行器、调度、通知属于高风险链路，修改时必须同步考虑结果、状态、日志。

### 前端边界
- 页面组件不得直接拼后端 URL，必须通过 `src/api/*` 统一封装。
- 动态路由、用户信息、token 相关逻辑只能在 router/store/utils 既有链路上扩展。
- 已经走真实后端接口的功能，不再补一套本地 JSON 假数据流程。

### 存储与配置边界
- 前端访问链路：`View -> src/api -> axios interceptor -> Backend API`
- 后端访问链路：`router/view -> module common/script/process -> model/database`
- 数据库存取优先通过 Tortoise model。
- 文件、媒体统一走项目既有路径体系。
- 配置优先收敛到现有配置文件，不散落硬编码。
- 新增后端运行配置统一进入 `l-tester-master/.env` 或 `.env.local`。
- 新增前端运行配置统一进入 `l-vue-ui-master/.env.*`。
- 后续三模式切换 `none / local_llm / remote_llm` 只能从统一配置层或网关层进入，不得散写到具体业务模块。

---

## 3. 工作流规则

### 默认工作方式
- 先读代码，再下手。
- 默认按 `Spec -> TDD -> Execution` 的顺序执行开发：先对齐需求和验收标准，再明确行为与失败场景，最后实现并回归验证。
- 对新功能、跨模块改动、高风险链路，优先做 spec 对齐，再写测试设计或验证清单，最后改业务代码。
- 遇到平台级重构任务时，必须先对齐阶段目标、范围边界和验收标准，再进入实现。

### 验证规则
- 后端改动：至少做导入级或调用级验证；能跑 pytest 就跑受影响范围。
- 前端改动：至少做 TypeScript 或构建级验证；能跑页面相关检查就跑受影响范围。
- 前后端联动：至少检查请求参数、响应结构、页面展示三者一致。
- 无法验证时必须明确说明原因，不能假装已经验证。

### TDD 最低门槛
- 新增后端业务接口：优先补 pytest 或最小调用级验证。
- 新增前端关键交互：优先补类型校验、构建校验，条件允许时补组件或页面级验证。
- 改登录、权限、调度、执行器、通知、报告：必须先列失败场景，再改实现。
- 没有自动化测试条件，不等于可以跳过测试设计。

---

## 4. 标准开场模板

### 推荐主模板

```text
请先读取项目根目录的 AGENTS.md，并严格按其中规定的顺序继续读取相关规则文件与项目记忆文件。

要求：
1. 先总结当前生效规则、范围边界、验收标准
2. 按 Spec/SDD -> TDD -> Execution -> VDD 流程推进
3. 如果不是小修小改，在我确认 Spec/SDD 前不要进入实现，在我确认 TDD 前不要开始改文件
4. 如进入具体目录，请继续读取最近的局部 AGENTS.md
5. 优先复用现有目录、接口封装、返回结构和工具函数
6. 所有规则文档、设计说明、交付说明统一使用中文
7. 读取文档、日志、控制台输出、配置文件时，统一按 UTF-8 处理
8. 如果出现中文乱码，先判断是终端输出编码问题还是文件本身编码问题，不要直接把乱码当成源文件损坏
9. 完成后按规则决定是否回写 current_phase、decision_log、module_memory、task_handoff、更新日志

先不要直接编码，先告诉我：
- 你读取了哪些规则文件
- 当前任务属于大改还是小修小改
- 你的 Spec/SDD
```

### 一句话短模板

```text
先读取根目录 AGENTS.md，并按要求继续读取 .cursor 规则、project-memory 和记忆文件；所有文档、日志、控制台输出统一按 UTF-8 处理，若出现乱码先区分终端显示问题与文件编码问题；先给我当前生效规则总结和 Spec/SDD，未经确认不要直接实现。
```

### 小修小改模板

```text
先读取根目录 AGENTS.md，并判断这次任务是否属于“小修小改”。如果是，请按最小 Spec/TDD/VDD 快速推进；如果不是，请回到标准流程。所有文档、日志、控制台输出按 UTF-8 处理；若出现乱码，先区分终端显示问题与源文件编码问题。先告诉我你的判断和依据，再开始。
```

### 大需求或重构模板

```text
先读取根目录 AGENTS.md，并严格按规则顺序读取项目规则与项目记忆。当前任务按非“小修小改”处理。所有文档、日志、控制台输出按 UTF-8 处理；若出现乱码，先区分终端显示问题与源文件编码问题。请先输出：
1. 当前生效规则
2. 本轮范围边界
3. 风险点
4. 验收标准
5. Spec/SDD

在我确认前不要进入 TDD 和 Execution。
```

### 个人固定版

```text
请先读取项目根目录的 AGENTS.md，并严格按其中规定的顺序继续读取相关规则文件与项目记忆文件。默认中文输出，默认将文档、日志、控制台输出、配置文件按 UTF-8 处理；若出现乱码，先判断是终端显示问题还是文件本身编码问题。按 Spec/SDD -> TDD -> Execution -> VDD 推进，未经我确认不要跨闸门。先告诉我当前生效规则、范围边界、验收标准和 Spec/SDD。
```

### 使用建议

- 开新对话默认优先使用“推荐主模板”
- 修 bug、改文案、局部兼容时优先使用“小修小改模板”
- 阶段优化、架构调整、规则重构时优先使用“大需求或重构模板”
- 若任务明确依赖官方文档、云平台配置或 SDK 最新约束，可在模板后追加“必要时优先使用 Context7”

---

## 5. 新项目规则迁移模板

### 新项目初始化模板

```text
这是一个新项目，目前还处于“规则初始化 / 架构落地前期”阶段，只有架构设计、目录骨架、部分设计文档，暂时没有完整业务代码。

请把现有规则体系迁移为这个新项目可用的规则骨架，但不要迁移旧项目的业务事实。

这个项目目前处于“规则初始化阶段”，不是“业务开发阶段”。请优先做规则适配，不要抢跑到代码实现。

要求：
1. 先读取项目根目录的 AGENTS.md，并把它作为唯一入口
2. 按 AGENTS.md 中定义的顺序继续读取规则文件、架构文档和项目记忆文件
3. 当前任务按“规则迁移与初始化”处理，不按“功能开发”处理
4. 保留通用方法论，只迁移这些内容：
   - 规则读取顺序
   - 规则优先级
   - Spec/SDD -> TDD -> Execution -> VDD 流程闸门
   - 文档回写机制
   - 项目记忆机制
   - 错误模式库 / 防复发机制
5. 只根据当前新项目的真实信息，替换这些内容：
   - 项目名称
   - 技术栈
   - 目录结构
   - 模块划分
   - 接口链路
   - 配置方式
   - 验证命令
   - 高风险链路
6. 删除不适用于当前项目的旧规则，不要把旧项目的业务模块、接口路径、页面职责、执行器能力、状态模型直接迁移过来
7. 如果当前项目还没有稳定的子目录边界，就先不要生成过细的局部 AGENTS.md
8. 所有规则文档、设计说明、交付说明统一使用中文
9. 读取文档、日志、控制台输出、配置文件时统一按 UTF-8 处理；如果出现乱码，先区分终端显示问题和文件编码问题
10. 未经我确认，不要直接改文件

先输出：
- 当前生效规则
- 你建议保留的通用规则
- 你建议按新项目重写的项目特有规则
- 你判断当前还缺少哪些关键信息
- Spec/SDD
```

### 新项目第二阶段微调模板

```text
当前项目的规则骨架已经初始化，请基于最新的项目架构、目录结构、模块边界和开发链路，对规则文件做第二轮微调适配。
这个项目目前处于“规则初始化阶段”，不是“业务开发阶段”。请优先做规则适配，不要抢跑到代码实现。

要求：
1. 仍然以根目录 AGENTS.md 为入口读取规则
2. 检查 .cursor/*.md、docs/project-memory/*.md，以及是否需要新增局部 AGENTS.md
3. 只根据当前项目已经确认的真实结构微调，不补写未经确认的业务事实
4. 重点收口：
   - 目录职责
   - 接口链路
   - 状态边界
   - 配置边界
   - 风险链路
   - 最低验证命令
   - 文档回写规则
5. 如果某些规则仍然过于通用，请明确指出哪些保留为全局模板，哪些应改成项目专用规则
6. 未经我确认，不要进入 Execution

先输出：
- 当前规则里不贴合项目的部分
- 建议新增的局部规则
- 建议删除或重写的规则
- Spec/SDD
```

### 新项目一句话极简版

```text
这是一个新项目，请以 AGENTS.md 为入口，把现有规则体系迁移为当前项目的规则骨架；保留方法论，不迁移旧项目业务事实，只按当前真实架构做适配，先给我 Spec/SDD，未经确认不要改文件。
```

### 新项目使用建议

- 新项目刚开始时优先使用“新项目初始化模板”
- 新项目开始出现真实代码和开发链路后，改用“新项目第二阶段微调模板”
- 项目进入正常开发阶段后，再回到上面的标准开场模板使用


